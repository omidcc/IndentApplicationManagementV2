...
namespace IndentApplicationManagement.Dal.Base
{
	public class UsersDalBase : SqlServerConnection
	{
		...
		{
			string sqlQuery ="Insert into Users (UserId, UserName, UserPassword, IsActive, UpdateBy, UpdateDate, CompanyId) values(@UserId, @UserName, @UserPassword, @IsActive, @UpdateBy, @UpdateDate, @CompanyId);";
			try
			{
		...
		{
			string sqlQuery = "Update Users set UserName = @UserName, UserPassword = @UserPassword, IsActive = @IsActive, UpdateBy = @UpdateBy, UpdateDate = @UpdateDate, CompanyId = @CompanyId where Users.UserId = @UserId;";
			try
			{
				int success = ExecuteNonQuery(sqlQuery, lstData);
				return success;
